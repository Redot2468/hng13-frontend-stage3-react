...
export default function EarphoneProducts() {
  const earphones = useQuery(api.category.getProductsByCategory, {
    category: "earphones",
  });

  const isLoading = earphones === undefined;
        ...
        >
          <div className="relative h-[352px] w-full overflow-hidden rounded-md lg:h-[560px] lg:w-1/2">
            <ImagesByViewport
              ...
              srcTab={earphone?.categoryImage?.tablet?.slice(1)}
              srcDesktop={earphone?.categoryImage?.desktop?.slice(1)}
              alt={earphone.description}
          ...
          <div className="flex flex-col items-center gap-6 text-center lg:w-1/2 lg:items-start lg:text-left">
            ...
            <h2 className="bold text-black uppercase">{earphone?.name}</h2>
