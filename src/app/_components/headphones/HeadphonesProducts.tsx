...
export default function HeadphonesProducts() {
        ...
        >
          <div className="relative h-[352px] w-full overflow-hidden rounded-md lg:h-[560px] lg:w-1/2">
            <ImagesByViewport
              ...
              srcTab={headphone?.categoryImage?.tablet?.slice(1)}
              srcDesktop={headphone?.categoryImage?.desktop?.slice(1)}
              alt={headphone.description}
          ...
          <div className="flex flex-col items-center gap-6 text-center lg:w-1/2 lg:items-start lg:text-left">
            ...
            <h2 className="bold text-black uppercase">{headphone?.name}</h2>
